# Multiselect Column Filters Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement SLB Optisite-style multiselect column filters with search, (Select All), and summary headers across all CBM tables in `CustomTable.tsx`.

**Architecture:** Update `src/components/CustomTable.tsx` to manage per-column filter selection state, popover visibility state, search query, and click-outside handlers. Table rows are filtered based on selected values per column.

**Tech Stack:** React, Next.js, Tailwind CSS, Lucide Icons (`SlidersHorizontal`, `Search`, `Check`, `Minus`, `X`).

## Global Constraints

- Header filter summary displays `(N) Item1,Item2,...` when specific options are selected, or blank when all items are selected.
- Icon: `SlidersHorizontal` placed inside the column filter bar.
- Popover contains a `Search...` input, `(Select All)` checkbox (supporting all-checked, unchecked, and `-` partial states), and a scrollable list of unique column value checkboxes.
- Popover closes on outside click.

---

### Task 1: Implement Multiselect Column Filter in CustomTable.tsx

**Files:**
- Modify: `src/components/CustomTable.tsx`

**Interfaces:**
- Consumes: Existing `columns`, `data`, `title`, `onRowClick` props of `CustomTable`.
- Produces: Enhanced `CustomTable` with SLB Optisite multiselect filtering behavior.

- [ ] **Step 1: Update CustomTable.tsx implementation**

Write the complete code for `src/components/CustomTable.tsx`:
                          ...
                          <div className="flex flex-col gap-1 max-h-48 overflow-y-auto pr-1">
                            ...
                            <label className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-bg-panel/40 cursor-pointer font-semibold select-none text-text-primary">
                              ...
                              <span className="text-xs">
                                {isSomeSelected ? '(Select All)' : isAllSelected ? '(Select All)' : '(Select All)'}
                              </span>
                            </label>
...
```bash
git add src/components/CustomTable.tsx
git commit -m "feat: add SLB Optisite multiselect column filters to CustomTable"
```
