# SLB Optisite Charts Alignment Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Align all chart components in `MetricCharts.tsx` with the SLB Optisite Figma templates (colors, top-left legends, bar stacking, callouts, and Y-axis scales).

**Architecture:** Update `src/components/MetricCharts.tsx` color constants, custom legend components, donut chart parameters, and stacked bar chart domain configurations.

**Tech Stack:** React, Next.js, Tailwind CSS, Recharts.

## Global Constraints

- Palette: Lime Green (`#84cc16`), Medium Blue (`#3b82f6`), Sky Blue (`#93c5fd`), Orange (`#f97316`), Red (`#f87171`), Slate Gray (`#475569`).
- Legend: Rendered at top-left with square swatches.
- Bar Charts: Y-axis ticks set to `0, 20, 40, 60, 80`.
- Donut Charts: `innerRadius={48}`, `outerRadius={70}` with exterior percentage callouts.

---

### Task 1: Update MetricCharts.tsx with Figma SLB Optisite Styling

**Files:**
- Modify: `src/components/MetricCharts.tsx`

**Interfaces:**
- Consumes: `EquipmentChartData`, `ChartWorkOrder` props of existing export functions:
  - `WorkOrderStatusPie`
  - `DaysLeftBar`
  - `EquipmentConditionPie`
  - `CbmCriticalityBar`
- Produces: Updated chart components styled strictly per the SLB Optisite Figma design.

- [ ] **Step 1: Write updated MetricCharts.tsx code**

Write the complete updated content for `src/components/MetricCharts.tsx`:
...
```bash
git add src/components/MetricCharts.tsx
git commit -m "feat: align charts with SLB Optisite Figma templates"
```
